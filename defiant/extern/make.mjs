...
export const exec_then_kill = (command, options) => {
	...
	try {
		handle = child_process.exec(command, options);
	} catch (err) {
		handle = null;
	}

	if (handle !== null && isNumber(handle.pid) === true) {
		...
		return new Promise((resolve, reject) => {
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					...
					if (handle.killed === true) {

						resolve(true);

					} else {

						console.error('> exec_then_kill("' + _(command) + '")');
